...
class ShowAPI(Resource):
    ...
    def post(self, venue_id):
        ...
        new_show = Show(venue_id=venue_id, Sname=show_name, Tags=tags, price=price,
                        timing=timing_datetime, tickets_remaining=venue.capacity, rating=0)
        try:
...
class BookingAPI(Resource):
    ...
    def post(self, show_id):
        ...
        if show.tickets_remaining >= no_of_tickets:
            booking = Booking(show_id=show_id, user_id=user_id,
    ...
    def patch(self, booking_id):
        ...
        if show.rating is not None:
            show.rating = ((show.rating * (no_of_bookings-1)) +
                           rating) / (no_of_bookings)
        else:
            show.rating = rating
        try:
            db.session.add(booking)
...
class SearchAPI(Resource):
    ...
    def get(self):
        ...
        shows = db.session.query(Show.show_id, Show.Sname, Show.rating, Show.Tags, Show.price, Show.timing, Show.tickets_remaining, Venue.Vname, Venue.place, Venue.venue_id, Venue.capacity).join(Venue).filter(
            Show.rating >= rating).filter(Venue.place.like(location)).filter(Show.Tags.ilike(tags)).filter(Show.Sname.ilike(show_name)).filter(Venue.Vname.ilike(venue_name)).filter(Show.timing >= selected_date).all()
...
class LocationAPI(Resource):
    ...
    def get(self):
        ...
        for location in distinct_locations:
            # Since each distinct location is a tuple
...
class RegistrationAPI(Resource):
    def post(self):
        ...
        user_datastore.create_user(
            email=email, password=password, fs_uniquifier=uuid.uuid4().hex)
        db.session.commit()
